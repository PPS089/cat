...
public class MediaFilesServiceImpl extends ServiceImpl<MediaFilesMapper, MediaFiles> implements MediaFilesService {
    ...
    public List<MediaFileVo> uploadMediaFiles(Integer recordId, MultipartFile[] files) {
        ...
        for (MultipartFile file : files) {
            ...
            MediaFileVo uploadedFile = processSingleFile(recordId, file, userId);
     ...
     * 处理单个文件上传
     */
    private MediaFileVo processSingleFile(Integer recordId, MultipartFile file, Long userId) {
        ...
        try {
            String targetFileName = buildObjectName(originalFilename, userId);
            String fileUrl = uploadToOss(file, targetFileName);
     ...
     * 上传文件到OSS
     */
    private String uploadToOss(MultipartFile file, String storedFilename) throws Exception {
        log.info("开始上传文件到OSS: {}", storedFilename);
        String fileUrl = ossOperator.upload(file.getBytes(), storedFilename);
        log.info("文件上传OSS成功: {} -> {}", storedFilename, fileUrl);
        return fileUrl;
    }
    ...
    private String buildObjectName(String originalFilename, Long userId) {
        ...
        Long owner = userId != null ? userId : 0L;
        return owner + "_" + System.currentTimeMillis() + "_" + UUID.randomUUID().toString().replace("-", "") + suffix;
    }
    ...
    public boolean deleteMediaByRecordId(Integer recordId) {
        ...
        if (result) {
            // 批量删除成功后，更新关联记录，让MyBatis Plus自动填充updateTime
            try {
                PetRecords petRecords = petRecordsMapper.selectById(recordId);
                if (petRecords != null) {
    ...
    public String uploadRawFile(MultipartFile file) {
        ...
        try {
            String targetFileName = buildObjectName(originalFilename, userId);
            return uploadToOss(file, targetFileName);
        } catch (Exception e) {
            log.error("通用文件上传失败: {}", e.getMessage(), e);
            throw new BizException(ErrorCode.MEDIA_FILE_UPLOAD_FAILED, "上传失败: " + e.getMessage());
        }
    }

    /**
     * 根据 URL 删除 OSS 文件（不涉及数据库记录）
     */
    @Override
    public void deleteByUrl(String url) {
        ...
        try {
            ossOperator.delete(url);
            log.info("已删除 OSS 文件: {}", url);
        } catch (Exception e) {
            log.warn("删除 OSS 文件失败: {}", url, e);
            throw new BizException(ErrorCode.MEDIA_FILE_DELETE_FAILED, "删除失败: " + e.getMessage());
        }
    }
