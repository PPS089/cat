...
public enum ErrorCode {
    ...
    EMAIL_EXISTS(400002, "邮箱已被注册", HttpStatus.BAD_REQUEST),
    USER_NOT_FOUND(404001, "用户不存在", HttpStatus.NOT_FOUND),
    PASSWORD_INCORRECT(401001, "用户名或密码错误", HttpStatus.UNAUTHORIZED),
    LOGIN_FAILED_USER_NOT_FOUND(401003, "用户不存在", HttpStatus.UNAUTHORIZED),
    ...
    OSS_CONFIG_ERROR(500013, "对象存储配置异常", HttpStatus.INTERNAL_SERVER_ERROR),
    OSS_OPERATION_FAILED(500014, "对象存储操作失败", HttpStatus.INTERNAL_SERVER_ERROR),
    ...
    METHOD_NOT_ALLOWED(405000, "请求方法不支持", HttpStatus.METHOD_NOT_ALLOWED),
    UNSUPPORTED_MEDIA_TYPE(415000, "媒体类型不支持", HttpStatus.UNSUPPORTED_MEDIA_TYPE),
