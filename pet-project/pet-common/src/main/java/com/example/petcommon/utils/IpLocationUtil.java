...
public class IpLocationUtil {
    ...
    private static final AtomicReference<byte[]> cBuffRef = new AtomicReference<>();
    private static final ThreadLocal<Searcher> TL = ThreadLocal.withInitial(() -> {
        try {
            ensureLoaded();
            byte[] cBuff = cBuffRef.get();
            if (cBuff == null || cBuff.length == 0) return null;
    ...
    public static class LocationInfo {
        ...
        public String getSimpleLocation() { 
            ...
            if (city != null && !city.isEmpty() && !city.equals(province)) sb.append(city); 
    ...
    public static String getLocationFromIP(String ipAddress) {
        ...
        return info != null ? info.getSimpleLocation() : "未知";
    }

    public static String getRawRegion(String ipAddress) {
        if (ipAddress == null || ipAddress.trim().isEmpty()) {
            return null;
        }
        try {
            ...
            return s.search(ipAddress);
        } catch (Exception e) {
            log.error("ip2region原始查询失败: {}", e.getMessage());
            return null;
        }
    }
    ...
    private static String norm(String s) {
        if (s == null) return "";
        s = s.trim();
        if ("0".equals(s) || "未知".equals(s)) return "";
        return s;
    }
}